...
struct UsersFixture {
	...
	MockDb db;
	UsersFixture()
	    : user(std::make_shared<User>("name", "pass", "email", "institute",
					  42)),
	      db() {}
};
...
BOOST_AUTO_TEST_CASE(SelectUserByName) {
	db.expect(
	    std::regex(R"(SELECT .* FROM users WHERE \(users.name='name'\))"));
	select_user(db, "name");
	db.validate();
}
...
BOOST_AUTO_TEST_CASE(SelectUserById) {
	db.expect(
	    std::regex(R"(SELECT .* FROM users WHERE \(users.userid=42\))"));
...
BOOST_AUTO_TEST_CASE(InsertBook) {
	...
	// t
	db.expect(
	    "INSERT INTO contents (bookid,pageid,lineid,seq,ocr,cor,cut,conf) "
...
BOOST_AUTO_TEST_CASE(SelectProject) {
	db.expect(
	    "SELECT project_pages.pageid FROM project_pages "
