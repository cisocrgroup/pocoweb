 ...
 */
static std::string
get_namespace_prefix(pugi::xml_node node);
...
new_curl()
{
  ...
  if (not curl)
    THROW(Error, "could not intialize curl");
  return curl;
}
...
RemoteProfiler::do_profile()
{
  ...
  slist.reset(curl_slist_append(slist.release(), "Content-Type: text/xml"));
...
////////////////////////////////////////////////////////////////////////////////
Profile
RemoteProfiler::parse_profile(const std::string& buffer, const Book& book)
{
  ...
  if (not ok) {
    THROW(
      Error, "(RemoteProfiler) could not parse response: ", ok.description());
  }
  const auto pre = get_namespace_prefix(doc.document_element());
  const auto ret =
    doc.document_element().child((pre + "returncode").data()).child_value();
  if (ret != std::string("0")) {
    THROW(Error, "(RemoteProfiler) invalid return code: ", ret);
  }
  const auto msg =
    doc.document_element().child((pre + "message").data()).child_value();
...
RemoteProfiler::do_languages()
{
  ...
  if (http_code != 200)
    THROW(Error, "invalid return code: ", http_code);
...
////////////////////////////////////////////////////////////////////////////////
std::vector<std::string>
RemoteProfiler::parse_languages(const std::string& data)
{
...
////////////////////////////////////////////////////////////////////////////////
size_t
RemoteProfiler::write(char* ptr, size_t size, size_t nmemb, void* userdata)
{
