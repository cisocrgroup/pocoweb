...
UserRoute::impl(HttpGet, const Request& req) const
{
  ...
  CROW_LOG_DEBUG << "(UserRoute) loading users";
...
UserRoute::impl(HttpDelete, const Request& req, int uid) const
{
  ...
  if (not session->user().admin() and user->id() != session->user().id()) {
    THROW(Forbidden,
          "user ",
          session->user().name,
          " cannot delete user ",
  ...
  auto ps = conn.db()(select(p.projectid).from(p).where(p.owner == user->id()));
  for (const auto& pp : ps) {
    ...
    if (project->is_book()) {
      ...
      CROW_LOG_INFO << "(UserRoute) removing directory: " << dir;
      boost::system::error_code ec;
      boost::filesystem::remove_all(dir, ec);
      if (ec)
        CROW_LOG_WARNING << "(UserRoute) cannot remove directory: " << dir
                         << ": " << ec.message();
    }
...
UserRoute::impl(HttpPost, const Request& req, int uid) const
{
  ...
  if (not session->user().admin() and session->user().id() != user->id()) {
    THROW(Forbidden,
          "user ",
          session->user().name,
          " cannot update user ",
