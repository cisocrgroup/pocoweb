...
public:
  ZipFile(const std::string &file) : file_(file) {
    system(("wget --quiet -nc http://www.cis.lmu.de/~finkf/pocoweb/" + file_)
               .data());
  }
  ~ZipFile() {
...
BOOST_AUTO_TEST_CASE(Ocropus) {
  ...
  BOOST_REQUIRE(line);
  BOOST_CHECK_EQUAL(line->ocr(), "ſlanti Finis alicujus propoſiti. Contrà, "
...
BOOST_AUTO_TEST_CASE(Alto) {
  ...
  BOOST_CHECK_EQUAL(p1->img,
                    dir / "rollenhagen_reysen_1603/rollenhagen_reysen_1603/"
                          "rollenhagen_reysen_1603_0007.tif");
  auto line = p1->find(1);
  BOOST_REQUIRE(line);
  BOOST_CHECK_EQUAL(line->ocr(), "Vnd daſſelb im Bluthroten Felde/");
  BOOST_CHECK_EQUAL(line->img,
                    dir / "line-images" / "0000000001" / "0000000001.png");
  ...
  BOOST_CHECK_EQUAL(line->img,
                    dir / "line-images" / "0000000002" / "0000000006.png");
  ...
  BOOST_CHECK_EQUAL(line->ocr(),
                    "in je einem Land ſo viel ſeltzame wunder wehrẽ/ wen");
  BOOST_CHECK_EQUAL(line->img,
                    dir / "line-images" / "0000000003" / "000000000e.png");
