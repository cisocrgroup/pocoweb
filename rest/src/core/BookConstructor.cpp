  ...
  for (const auto &ocr : ocr_) {
    ...
    while (pp->has_next()) {
      ...
      if (page->has_ocr_path() and not page->has_img_path()) {
        ...
        if (i == end(img_))
          CROW_LOG_WARNING << "(BookConstructor) cannot find "
                              "image file for: "
                           << page->ocr;
      }
...
void BookConstructor::set_img_file(Page &page) const {
  ...
  if (not page.has_img_path()) {
    CROW_LOG_WARNING << "(BookConstructor) cannot find image file for: "
                     << page.ocr;
  } else {
...
BookConstructor::find_matching_img_file(const Path &path) const noexcept {
  ...
  f = std::find_if(begin(img_), end(img_),
                   [&](const auto &ipath) { return ipath.filename() == fn; });
  if (f != end(img_))
    return f;
