...
////////////////////////////////////////////////////////////////////////////////
std::string pcw::base64::encode(const ::std::string &bindata) {
	using ::std::string;
	...
	if (bindata.size() >
		...
		throw ::std::length_error(
		    "Converting too large a string to base64.");
	}
...
////////////////////////////////////////////////////////////////////////////////
std::string pcw::base64::decode(const ::std::string &ascdata) {
	using ::std::string;
	string retval;
	...
	for (string::const_iterator i = ascdata.begin(); i != last; ++i) {
		const int c = *i;
		if (::std::isspace(c) || c == '=') {
			// Skip whitespace and padding. Be liberal in what you
			// accept.
			continue;
		}
		if ((c > 127) || (c < 0) || (reverse_table[c] > 63)) {
			throw ::std::invalid_argument(
			    "This contains characters not legal in a base64 "
			    "encoded string.");
		}
		...
		if (bits_collected >= 8) {
			bits_collected -= 8;
			retval += static_cast<char>(
			    (accumulator >> bits_collected) & 0xffu);
		}
	}
	return retval;
}
