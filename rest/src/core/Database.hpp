...
namespace pcw {
	...
	public:
		...
		UserPtr insert_user(const std::string& name,
				const std::string& pass) const;
		UserPtr authenticate(const std::string& name,
				const std::string& pass) const;
		...
		void update_book(const BookView& view) const;
	...
	private:
		UserPtr select_user(const std::string& name,
		...
		BookViewPtr select_subproject(int projectid, int origin, int owner,
				sql::Connection& conn) const;
		BookViewPtr select_subproject(int projectid, int owner,
				const Book& book, sql::Connection& conn) const;
		BookPtr select_book(int bookid, int owner,
				sql::Connection& conn) const;
		int insert_book_project(const BookView& project,
				sql::Connection& conn) const;
		...
		UserPtr cached_select_user(const std::string& name,
				sql::Connection& conn) const;
		UserPtr cached_select_user(int userid, sql::Connection& conn) const;
		BookViewPtr cached_select_project(int prid,
				sql::Connection& conn) const;
