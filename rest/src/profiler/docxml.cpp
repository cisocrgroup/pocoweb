...
{
private:
  DocXmlNode(const pugi::xml_node& node, const Profile::Suggestions* suggs)
    : pugi::xml_node(node)
    , suggestions(suggs)
  {}
...
////////////////////////////////////////////////////////////////////////////////
static void
append_candidates(pugi::xml_node& node,
                  const Token& token,
...
namespace pcw {
DocXmlNode&
operator<<(DocXmlNode& j, const Page& page);
DocXmlNode&
operator<<(DocXmlNode& j, const Line& line);
DocXmlNode&
operator<<(DocXmlNode& j, const Token& token);
DocXmlNode&
operator<<(DocXmlNode& j, const Box& Box);
}
...
{
  line.each_token([&node](const auto& token) {
    if (token.is_normal())
      node << token;
  });
  return node;
...
pcw::operator<<(DocXmlNode& node, const Token& token)
{
  ...
  tnode.append_attribute("isNormal").set_value(bools[(int)!!token.is_normal()]);
