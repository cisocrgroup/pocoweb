...
class Corrector {
       public:
	Corrector(ProjectPtr view = nullptr) : wf_(), view_(std::move(view)){};
	void set_view(ProjectPtr view) noexcept { view_ = std::move(view); }
	const WagnerFischer& wf() const noexcept { return wf_; }
	WagnerFischer& wf() noexcept { return wf_; }

	void correct(int pid, int lid, const std::wstring& str);
	void correct(int pid, int lid, const std::string& str);

	void correct(int pid, int lid, const std::wregex& pat,
		     const std::wstring repl);
	void correct(int pid, int lid, const std::wstring& pat,
		     const std::wstring repl);
	void correct(int pid, int lid, const std::string& pat,
