...
////////////////////////////////////////////////////////////////////////////////
Route::Response Logout::login(const Request& req, const std::string& name,
	...
	if (session) {
		CROW_LOG_INFO << "(Logout) existing session sid: "
			      << session->id();
		return ok();
	}
	...
	if (not user->password.authenticate(pass)) {
		CROW_LOG_ERROR << "(Logout) invalid password for user: "
			       << name;
	}
