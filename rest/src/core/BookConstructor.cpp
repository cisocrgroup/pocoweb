...
////////////////////////////////////////////////////////////////////////////////
BookPtr BookConstructor::build() const {
...
////////////////////////////////////////////////////////////////////////////////
void BookConstructor::add(const Path& file) {
...
////////////////////////////////////////////////////////////////////////////////
BookConstructor::BookInfo BookConstructor::parse_book_info() const {
	BookInfo info;
...
////////////////////////////////////////////////////////////////////////////////
void BookConstructor::set_img_file(Page& page) const {
	...
	if (not page.has_img_path()) {
		...
		if (img != end(img_)) page.img = *img;
	}
	if (not page.has_img_path()) {
		CROW_LOG_WARNING
		    << "(BookConstructor) Cannot find image file for: "
		    << page.ocr;
	} else {
		CROW_LOG_INFO << "(BookConstructor) Found page: " << page.ocr
			      << " with image file: " << page.img;
	}
}

////////////////////////////////////////////////////////////////////////////////
BookPtr BookConstructor::build(const BookInfo& info) {
	BookData data;
	...
	for (auto& page : info.pages) builder.append(*page);
...
BookConstructor::find_matching_img_file(const Path& path) const noexcept {
	...
	auto f = std::find_if(begin(img_), end(img_),
			      [&](const auto& ipath) { return ipath == path; });
	if (f != end(img_)) return f;
...
////////////////////////////////////////////////////////////////////////////////
void BookConstructor::fix_indizes(std::vector<PagePtr>& pages) noexcept {
	int id = 0;
	for (auto& page : pages) {
		page->set_id(++id);
	}
