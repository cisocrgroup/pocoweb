...
private:
  Response correct(MysqlConnection &conn, int pid, int p, Line &line,
                   const std::string &c) const;
  Response correctx(MysqlConnection &conn, int pid, int tid, Line &line,
                    const std::string &c) const;
...
public:
  Corrector(MysqlConnection &c, int pid, int pageid, int lid)
      : c_(c), pid_(pid), pageid_(pageid), lid_(lid), origin_() {}
