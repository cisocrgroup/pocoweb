...
Profile LocalProfiler::do_profile() {
	...
	CROW_LOG_INFO << "(LocalProfiler) Done profiling book id: "
		      << book().id();
	return read_profile();
}
...
void LocalProfiler::run_profiler(const std::string& command) const {
	CROW_LOG_DEBUG << "(LocalProfiler) Executing profiler command: "
		       << command;
	...
	switch (res) {
		case -1:
			throw std::system_error(errno, std::system_category(),
						"fork");
		case 127:
			THROW(Error, "Shell could not execute profiler: ",
			      command);
		case 0:
			break;
		default:
...
void LocalProfiler::write_docxml() const {
	...
	docxml.save(os);
	os.close();
	CROW_LOG_DEBUG << "(LocalProfiler) Done writing doc xml file "
		       << outfile_;
}
