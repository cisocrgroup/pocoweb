...
void WagnerFischer::set_gt(const std::wregex &pat, const std::wstring &repl) {
  if (gt_.empty()) {
    std::regex_replace(std::back_inserter(gt_), begin(ocr_), end(ocr_), pat,
                       repl.data());
  } else {
    std::wstring tmp = gt_;
    gt_.clear();
    std::regex_replace(std::back_inserter(gt_), begin(tmp), end(tmp), pat,
                       repl.data());
  }
}
...
////////////////////////////////////////////////////////////////////////////////
void WagnerFischer::set_ocr(const Line &line) { ocr_ = line.wocr(); }
...
size_t WagnerFischer::operator()(size_t b, size_t n) {
  ...
  std::cerr << "WagnerFischer::operator()(" << b << "," << n << ")\n";
  // update gt: ...xxx...
  std::cerr << "gt(before)  = " << utf8(gt_) << "\n";
  std::cerr << "ocr(before) = " << utf8(ocr_) << "\n";
  gt_ = ocr_.substr(0, b) + gt_ + ocr_.substr(b + n);
  std::cerr << "gt(after)  = " << utf8(gt_) << "\n";
  std::cerr << "ocr(after) = " << utf8(ocr_) << "\n";
  return (*this)();
}
...
WagnerFischer::backtrack(size_t i, size_t j) const noexcept {
  ...
  case 0:
    return x[0] == l_[i][j] ? std::make_tuple(EditOp::Nop, i - 1, j - 1)
...
pcw::WagnerFischer::find_corrected_token(size_t i) const noexcept {
  ...
  auto f =
      std::find_if_not(b + i, e, [](auto op) { return op == EditOp::Nop; });
  if (f != e)
