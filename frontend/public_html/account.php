...
if (isset($_GET["update"])) {
    if (!isset($_POST["name"]) || !isset($_POST["email"]) ||
        ...
        $pass = $_POST["pass1"] == "__NULL__" ? "" : $_POST["pass1"];
        $api = backend_update_user(
            $SID["user"]["id"],
            $_POST["name"],
            $_POST["email"],
            $_POST["institute"],
...
if (isset($_GET["delete"])) {
    $api = backend_delete_user($SID["user"]['id']);
    $status = $api->get_http_status_code();
    if ($status == 200) {
	...
	frontend_render_account_div($SID["user"]);
} else {
