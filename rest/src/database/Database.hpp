...
template<class Row>
BookData
make_book_data(const Row& row) noexcept;

template<class Row>
ProjectEntry
make_project_entry(const Row& row) noexcept;

template<class Db, class P, class Q, class R>
void
insert_page(Db& db, P& p, Q& q, R& r, const Page& page);

template<class Db, class Q, class R>
void
insert_line(Db& db, Q& q, R& r, const Line& line);

template<class Db, class R>
void
insert_content(Db& db, R& r, const Line& line);

template<class Db>
void
insert_book_info_and_set_id(Db& db, Book& book);

template<class Db>
void
insert_project_info_and_set_id(Db& db, Project& view);

template<class Db>
void
delete_project(Db& db, int pid);
...
template<class Db>
void
select_lines(Db& db, const PageBuilder& builder, int bookid, int pageid);

template<class Db>
void
select_contents(Db& db,
                const LineBuilder& builder,
                int bookid,
                int pageid,
                int lineid);
} // namespace detail
...
template<class Db>
void
update_book(Db& db, Project& view);
...
template<class Db>
boost::optional<ProjectEntry>
select_project_entry(Db& db, int projectid);
...
select_book_data(Db& db, int bookid);
...
template<class Db>
void
update_project_owner(Db& db, int projectid, int owner);
...
template<class Db>
ProjectSptr
select_project(Db& db, const Book& book, int projectid);

template<class Db>
void
update_line(Db& db, const Line& line);
} // namespace pcw
...
pcw::insert_project(Db& db, Project& view)
{
  ...
  auto stmnt =
    db.prepare(insert_into(project_pages)
...
pcw::insert_book(Db& db, Book& book)
{
  ...
  tables::Books books;
  auto stmnt = insert_into(books).set(books.author = book.data.author,
...
pcw::select_book_data(Db& db, int bookid)
{
  ...
  tables::Status s;
  auto stmnt =
    select(all_of(b), s.text)
      .from(b.join(p).on(b.bookid == p.origin).join(s).on(s.id == b.statusid))
...
pcw::detail::select_pages(Db& db, const BookBuilder& builder, int bookid)
{
  ...
  for (const auto& row : db(stmnt)) {
    ...
    detail::set_if_not_null(
      row.imagepath, [&](const auto& p) { pbuilder.set_image_path(Path(p)); });
    detail::set_if_not_null(
                          ...
                          int pageid)
{
  ...
  for (const auto& row : db(stmnt)) {
    lbuilder.reset();
    detail::set_if_not_null(
      row.imagepath, [&](const auto& p) { lbuilder.set_image_path(Path(p)); });
...
template<class Db>
void
pcw::detail::select_contents(Db& db,
                             const LineBuilder& builder,
                             int bookid,
                             int pageid,
                             int lineid)
