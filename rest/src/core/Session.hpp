...
class Session {
	...
	template <class Db>
	inline std::vector<ProjectSptr> select_all_projects(
	    Connection<Db>& c) const;
	template <class Db>
	...
	template <class Db>
	inline UserSptr find_user(Connection<Db>& c,
	...
	template <class Db>
	UserSptr cached_find_user(Connection<Db>& c,
				  const std::string& name) const;
	template <class Db>
	UserSptr find_user_impl(Connection<Db>& c, int userid) const;
	template <class Db>
	UserSptr find_user_impl(Connection<Db>& c,
				const std::string& name) const;
...
////////////////////////////////////////////////////////////////////////////////
template <class Db>
std::vector<pcw::ProjectSptr> pcw::Session::select_all_projects(
    Connection<Db>& c) const {
...
pcw::LineSptr pcw::Session::find(Connection<Db>& c, int bookid, int pageid,
	...
	if (page) return page->find(lineid);
	return nullptr;
}
...
void pcw::Session::insert_project_impl(Connection<Db>& c, Project& view) const {
	if (view.is_book()) {
		auto book =
		    std::dynamic_pointer_cast<Book>(view.shared_from_this());
...
inline pcw::ProjectSptr pcw::Session::find_project_impl(Connection<Db>& c,
	...
	if (entry and entry->is_book()) {
		...
		if (not owner) return nullptr;
		auto book = cached_find_book(c, entry->origin);
		if (not book) return nullptr;
		assert(book->is_book());
...
////////////////////////////////////////////////////////////////////////////////
template <class Db>
inline pcw::UserSptr pcw::Session::find_user(Connection<Db>& c,
					     const std::string& name) const {
...
////////////////////////////////////////////////////////////////////////////////
template <class Db>
inline pcw::UserSptr pcw::Session::find_user_impl(
    Connection<Db>& c, const std::string& name) const {
	return select_user(c.db(), name);
}

////////////////////////////////////////////////////////////////////////////////
template <class Db>
inline pcw::UserSptr pcw::Session::find_user_impl(Connection<Db>& c,
						  int userid) const {
...
////////////////////////////////////////////////////////////////////////////////
template <class Db>
pcw::UserSptr pcw::Session::cached_find_user(Connection<Db>& c,
					     const std::string& name) const {
	if (cache_)
...
pcw::UserSptr pcw::Session::cached_find_user(Connection<Db>& c,
					     int userid) const {
	if (cache_)
		return cache_->users.get(
		    userid, [&](int id) { return find_user_impl(c, id); });
	else
