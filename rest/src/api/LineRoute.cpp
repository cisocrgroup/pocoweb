...
Route::Response LineRoute::impl(HttpPost, const Request& req, int bid, int pid,
	...
	if (not json["d"].s().size())
		THROW(BadRequest, "missing data for book id: ", bid,
	...
	auto line = session->find(conn, bid, pid, lid);
	if (not line)
		THROW(NotFound, "Cannot find book id: ", bid, " page id: ", pid,
		      " line id: ", lid);
...
////////////////////////////////////////////////////////////////////////////////
void LineRoute::log(const WagnerFischer& wf) const {
	std::string u8;
...
////////////////////////////////////////////////////////////////////////////////
void LineRoute::print_with_dotted_circles(const std::wstring& str,
					  std::string& u8) {
	static const wchar_t dotted_circle = 0x25cc;
	u8.clear();
	for (auto c : str) {
		if (u_charType(c) == U_NON_SPACING_MARK)
			utf8::utf32to8(&dotted_circle, &dotted_circle + 1,
