...
void SuspiciousWordsRoute::Register(App& app) {
	CROW_ROUTE(app, SUSPICIOUS_WORDS_ROUTE_ROUTE_1)
	    .methods("GET"_method)(*this);
	CROW_ROUTE(app, SUSPICIOUS_WORDS_ROUTE_ROUTE_2)
	    .methods("GET"_method)(*this);
	CROW_ROUTE(app, SUSPICIOUS_WORDS_ROUTE_ROUTE_3)
	    .methods("GET"_method)(*this);
}

////////////////////////////////////////////////////////////////////////////////
SuspiciousWordsRoute::Response SuspiciousWordsRoute::impl(HttpGet,
							  const Request& req,
							  int bid) const {
	CROW_LOG_DEBUG
	...
	tables::Types t;
	auto rows =
	    conn.db()(select(t.string, s.pageid, s.lineid, s.tokenid)
