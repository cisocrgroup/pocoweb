...
static inline Box operator+(const Box &a, const Box &b) noexcept {
  auto box = a;
  return box += b;
}

static inline bool operator==(const Box &a, const Box &b) noexcept {
...
template <class OS> void Box::serialize(rapidjson::Writer<OS> &w) const {
  w.StartObject();
  w.String("left");
  w.Int(left());
  w.String("right");
