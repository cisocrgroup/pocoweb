...
namespace pcw {
	...
	class Connection {
	public:
		Connection(ConnectionPoolConnection<Db>* c): c_(c) {}
		...
		Connection(Connection&& o) noexcept: c_(o.c_)
		{
			o.c_ = nullptr;
		}
	...
	private:
		...
		template<size_t... i>
		inline connection_uptr_t new_connection(
...
template<class Db, class... A>
pcw::Connection<Db>
pcw::ConnectionPool<Db, A...>::get_connection(connection_uptr_t& connection) const
{
	if (not connection)
